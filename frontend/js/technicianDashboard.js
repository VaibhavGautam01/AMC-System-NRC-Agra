...
// ================================
// Load System Announcements (DYNAMIC COLOR RE-MAPPING MATRIX)
// ================================
async function checkSystemAnnouncements() {
    try {
        ...
        if (data.success && data.announcement) {
            ...
            // Decide background color class dynamically based on presence of PDF link
            const wagonColorClass = data.pdf_url ? 'wagon-blue' : 'wagon-green';

            wagonsWrapper.innerHTML =
                coaches.map(text => `
                    <div class="train-wagon ${wagonColorClass}">
                         ${text}
                    </div>
                `).join("") +
                ...
                (data.pdf_url
                    ? `
                    <div class="train-wagon wagon-blue">
